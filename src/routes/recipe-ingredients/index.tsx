        ...
        <main className="">
          <section className="mx-auto max-w-6xl p-6 gap-6 py-12">
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <h1 className="text-3xl font-semibold font-[family-name:var(--font-display)]">Recipe Ingredients</h1>
              <Input
            ...
            {isPending ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filtered.map((row: Record<string, unknown>) => (
                  <Link key={String(row.id)} to={"/recipe-ingredients/" + String(row.id)} className="group">
                    <Card className="rounded-[0.5rem] border border-border shadow-sm transition-all duration-200 hover:scale-[1.02] overflow-hidden">
                      <img src={`https://picsum.photos/seed/recipe_ingredients-${String(row.id).slice(0,8)}/400/300`} alt="" className="h-48 w-full object-cover" />
                      <CardHeader>
                      ...
                      <CardContent className="space-y-1"><p className="text-sm text-muted-foreground">{String(row.recipe_id ?? '—')}</p>
                        <p className="text-sm text-muted-foreground">{String(row.quantity ?? '—')}</p></CardContent>
                    </Card>
                  </Link>
                ))}
              </div>
